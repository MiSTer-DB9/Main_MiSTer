...
static void user_io_joyraw_check_change()
{
	...
	// it stays 0 for a configured-but-untouched pad. On a core (re)load, seed the
	// type from the user's saved/OSD selector so a configured-but-untouched pad
	// still streams its table (otherwise the joydb_remap reset state -- all-NONE --
	// leaves every button dead in-game until the pad is moved once inside the OSD).
	// Resolving the selector ONLY on the load edge -- not every poll -- keeps the
	// CONF_STR walk out of the 20Hz gameplay path (Critical Rule #2) and stops the
	// table flapping between the live-detected and the configured type when the two
	// disagree; once seeded, live detection (when present) stays authoritative.
	int devtype = (joyraw >> 14) & 3;
	static int last_map_devtype = 0;
	if (db9_map_pending)
	{
		// New core: forget the previous core's applied type and seed from the
		// saved selector when the pad isn't being live-detected this poll.
		last_map_devtype = 0;
